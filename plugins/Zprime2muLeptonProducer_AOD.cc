...
class Zprime2muLeptonProducer_AOD : public edm::EDProducer {
public:
  explicit Zprime2muLeptonProducer_AOD(const edm::ParameterSet&);
...
Zprime2muLeptonProducer_AOD::Zprime2muLeptonProducer_AOD(const edm::ParameterSet& cfg)
...
pat::Electron* Zprime2muLeptonProducer_AOD::cloneAndSwitchElectronEnergy(const pat::Electron& electron) const {
...
pat::Muon* Zprime2muLeptonProducer_AOD::cloneAndSwitchMuonTrack(const pat::Muon& muon) const {
...
void Zprime2muLeptonProducer_AOD::embedTriggerMatch(pat::Muon* new_mu, const std::string& ex, const trigger::TriggerObjectCollection& L3, std::vector<int>& L3_matched) {
  int best = -1;
...
std::pair<pat::Electron*,int> Zprime2muLeptonProducer_AOD::doLepton(const edm::Event& event, const pat::Electron& el, const reco::CandidateBaseRef&) {
...
std::pair<pat::Muon*,int> Zprime2muLeptonProducer_AOD::doLepton(const edm::Event& event, const pat::Muon& mu, const reco::CandidateBaseRef& cand) {
...
template <typename T>
edm::OrphanHandle<std::vector<T> > Zprime2muLeptonProducer_AOD::doLeptons(edm::Event& event, const edm::InputTag& src, const std::string& instance_label) {
...
void Zprime2muLeptonProducer_AOD::produce(edm::Event& event, const edm::EventSetup& setup) {
  ...
  if (electron_muon_veto_dR > 0) {
    if (!muons.isValid())
      throw cms::Exception("Zprime2muLeptonProducer_AOD") << "requested to veto electrons near muons, but main muon collection is invalid!\n";
...
DEFINE_FWK_MODULE(Zprime2muLeptonProducer_AOD);
